 ...
 */
 var pianoInput = [];
 var code = "<YOUR CODE>";
 var code_length = <CODE LENGTH (as Integer)>;
    ...
    (function() {
        ...
        $keys.on('played-note.piano', function(evt, key, $elt) {
            ...
            var eOffset = $elt.offset(),
                ...
                pianoInput.push((key) + noteMap[key + ""]);
                if (pianoInput.length == code_length) {
                    var joinedInput = pianoInput.join("");
                    if (code === joinedInput) {
                        var audio = document.getElementById("audioplayer"); 
                        audio.play();
                        $.post("/open", {code: joinedInput});
                    }
                    pianoInput = pianoInput.slice(1, pianoInput.length + 1)
                }
